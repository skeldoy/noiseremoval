...
def generate_text(model, start_text='The', max_length=1000, temperature=0.5):
    model.eval()
    with torch.no_grad():
...
def main():
    # Parameters
    model_path = 'char_gru_model.pth'  # Ensure the path matches your saved model
    input_size = output_size = len(dataset.chars)
    hidden_size = 256  # Ensure this matches the trained model's hidden size
    num_layers = 2  # Ensure this matches the trained model's number of layers
